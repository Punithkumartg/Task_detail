...
public class scanspage {
    ...
    public void pageoption_click() {
        Locator el = page.getByRole(AriaRole.COMBOBOX, new Page.GetByRoleOptions());
        //page.locator("[role='combobox']");
        el.click();
        el.screenshot(new Locator.ScreenshotOptions().setPath(Paths.get("D:\\IntelIJworkspace\\Playwright_testNG\\screenshot")));

    }

    public void pagination_option() {
        List<ElementHandle> pagination_option = page.querySelectorAll(".rc-virtual-list");
        for (ElementHandle options : pagination_option) {

            for (int i = 0; i <= 4; i++) {
                options.click();
                System.out.println(options.getAttribute("title"));
            }
        }
    }
...
{
    page.locator(".ant-card-body >div .ant-space-item:nth-child(4) span").hover();
}
public void additional_columnhover()
{
    page.locator(".ant-card-body >div .ant-space-item:nth-child(6) span").hover();
}

public void empty_file_assertion()
{
